...
@pytest.fixture
def staff_user(db):
    return User.objects.create_user(username="admin", password="adminpass", is_staff=True, is_active=True)
...
def test_api_create_node(auth_api_client):
    response = auth_api_client.post("/api/nodes/", {
        "name": "API Node",
        "email": "api@node.com",
        "country": "Беларусь",
        "city": "Минск",
...
def test_api_update_node(auth_api_client, node):
    ...
    response = auth_api_client.patch(url, {
        "city": "Новосибирск",
        "debt": "0"  # должно остаться без изменений (запрет)
    })
